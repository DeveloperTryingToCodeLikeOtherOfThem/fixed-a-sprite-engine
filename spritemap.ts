namespace sprites2 {
    export class SpriteMap {
        ...
        private buckets: Sprite2[][]; // <-- use Sprite2 here
        ...
        overlaps(sprite: Sprite2): Sprite2[] {
            ...
            return n.filter(neighbor => sprite.overlapsWith(neighbor));
        }
        ...
        update(sprites: Sprite2[]) {
            ...
            const tMap = game2.currentScene().tileMap;
        ...
        private key(x: number, y: number): number {
            const xi = Math.clamp(0, this.columnCount - 1, Math.idiv(x, this.cellWidth));
            const yi = Math.clamp(0, this.rowCount - 1, Math.idiv(y, this.cellHeight));
        ...
        private insertAABB(sprite: Sprite2) {
            if (this.isOob(sprite) || (sprite.flags & sprites2.Flag.Ghost))
                return;
