...
namespace sprites2 {
     ...
     */
    //% group="Create"
    //% blockId=spritescreate2 block="sprite %img=screen_image_picker of kind %kind=spritetype"
     ...
     */
    //% blockId=allOfKind2 block="array of sprites of kind %kind=spritetype"
    //% blockNamespace="arrays" blockSetVariable="sprite list"
    //% weight=87
    export function allOfKind(kind: number): Sprite2[] {
     ...
     */
    //% group="Projectiles"
    //% blockId=spritescreateprojectilefromside2 block="projectile %img=screen_image_picker from side with vx %vx vy %vy"
     ...
     */
    //% group="Projectiles"
    //% blockId=spritescreateprojectilefromsprite2 block="projectile %img=screen_image_picker from %sprite=variables_get(mySprite) with vx %vx vy %vy"
    //% vx.shadow=spriteSpeedPicker
    //% vy.shadow=spriteSpeedPicker
    ...
    //% inlineInputMode=inline
    //% vy.defl=100
    export function createProjectileFromSprite(img: Image, sprite: Sprite2, vx: number, vy: number): Sprite2 {
     ...
     */
    //% group="Projectiles"
    //% blockId=spritescreateprojectile2 block="projectile %img=screen_image_picker vx %vx vy %vy of kind %kind=spritetype||from sprite %sprite=variables_get(mySprite)"
    ...
    export function createProjectile(img: Image, vx: number, vy: number, kind?: number, sprite?: Sprite2) {
        const s = sprites2.create(img, kind);
        const sc = game2.currentScene();
        s.vx = vx
        s.vy = vy
        ...
        s.flags |= sprites2.Flag.AutoDestroy;
        s.flags |= sprites2.Flag.DestroyOnWall;
